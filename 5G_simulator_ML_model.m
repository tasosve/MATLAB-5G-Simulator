...
load('RF_handover_model.mat', 'RF_handover');
load('RF_best_gNB_model.mat', 'RF_best_gNB');

%% PARAMETERS

datasetFolder = 'datasetRF18';
...
for runIdx = 1:numRuns
    ...
    pastSINRWindow = 3; % Number of past SINR values as features
    sinrHistBuffer = nan(numUE, pastSINRWindow); % Initialize with NaNs or zeros
    ...
    LOSState = true(num_gNB, numUE); % start with all LOS=true, can also randomize if you want
    ...
    for t=1:numSteps
        for u=1:numUE
            ...
            % Extract features
            sinr_val = SINRLog(t,u);
            servingCellName = servingCellLog{max(t-1,1), u}; % extract string
            servingCell_val = sscanf(servingCellName, 'gNB%d'); % convert to numeric

            num_gNB_current = size(gNB_pos,1);

            dist_vals = zeros(1, num_gNB_current);
            power_vals = zeros(1, num_gNB_current);
            
            for g = 1:num_gNB_current
                dist_vals(g) = distances(t,u,g);
                power_vals(g) = rxPowersAll(t,u,g);
            end
           
            % Update SINR history buffer before prediction
            if t == 1
                sinrHistBuffer(u,:) = nan(1, pastSINRWindow); % or fill with sinr_val if preferred
            else
                sinrHistBuffer(u,1:end-1) = sinrHistBuffer(u,2:end);
                sinrHistBuffer(u,end) = SINRLog(t-1,u);
            end

            X_test = [servingCell_val, power_vals, sinr_val, sinrHistBuffer(u,:)];
            
            [handover_label, handover_scores] = predict(RF_handover, X_test);
            [best_gNB_label, best_gNB_scores] = predict(RF_best_gNB, X_test);
            
            handover_event = str2double(handover_label{1});
            p_handover = handover_scores(strcmp(RF_handover.ClassNames, '1'));
            
            predicted_best_gNB = str2double(best_gNB_label{1});
            
            if p_handover > 0.9 && predicted_best_gNB ~= servingCell_val
                servingCell = predicted_best_gNB;
                handoverEvents(t,u) = 1;
            else
                servingCell = sscanf(servingCellLog{max(t-1,1), u}, 'gNB%d');
                handoverEvents(t,u) = 0;
            end
            
            servingCellLog{t,u} = sprintf('gNB%d', servingCell);
            
            % Debug print for handover decisions:
            %fprintf('t=%d UE=%d p_ho=%.2f handover=%d servingCell=%d best_gNB=%d\n', ...
            %    t, u, p_handover, handoverEvents(t,u), servingCell, predicted_best_gNB);
    ...
    filename = fullfile(datasetFolder, sprintf('RF_simulation_data_run%d.mat', runIdx+10));
    ...
    csvFilename = fullfile(datasetFolder, sprintf('RF_simulation_metrics_run%d.csv', runIdx+10));
